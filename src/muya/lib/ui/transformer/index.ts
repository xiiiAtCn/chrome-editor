...
class Transformer {
  ...
  listen() {
    ...
    eventCenter.attachDOMEvent((document as any) as HTMLElement, 'click', this.hide.bind(this))
    eventCenter.subscribe(
      'muya-transformer',
      ({ reference, imageInfo }: { reference: HTMLElement; imageInfo: Block }) => {
  ...
  update() {
    ...
    CIRCLES.forEach(c => {
      const circle = this.container.querySelector(`.${c}`) as HTMLElement
  ...
  mouseDown = (event: MouseEvent) => {
    ...
    this.eventId.push(mouseMoveId as string, mouseUpId as string)
  }
  ...
  mouseMove = (event: MouseEvent) => {
    ...
    switch (this.movingAnchor) {
      case 'top-left':
      case 'bottom-left':
        relativeAnchor = this.container.querySelector('.top-right') as HTMLElement
        width = Math.max(relativeAnchor.getBoundingClientRect().left + CIRCLE_RADIO - clientX, 50)
        break
      case 'top-right':
      case 'bottom-right':
        relativeAnchor = this.container.querySelector('.top-left') as HTMLElement
  ...
  mouseUp = (event: MouseEvent) => {
    ...
    if (typeof this.width === 'number') {
      this.muya.contentState.updateImage(this.imageInfo!, 'width', String(this.width))
      this.width = null
      this.hide()
    }
    this.resizing = false
