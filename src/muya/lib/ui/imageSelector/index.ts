...
import { Block, IState } from '@/typings/muya'
import { createApi } from 'unsplash-js'
import Muya from '../..'
import { EVENT_KEYS, isWin, URL_REG } from '../../config'
import { h, patch } from '../../parser/render/snabbdom'
import { getImageInfo as getImageSrc, getUniqueId } from '../../utils'
...
class ImageSelector extends BaseFloat {
  ...
  oldVnode: any
  imageInfo: Block | null

  unsplash!: ReturnType<typeof createApi>

  photoList: Array<any> | undefined
  ...
  state: IState
  imageSelectorContainer: HTMLElement

  options!: IFloatOption & {
    unsplashAccessKey: string
    photoCreatorClick?: (str: string) => void
  }
  ...
  constructor(
    muya: Muya,
    options: IFloatOption & { unsplashAccessKey: string; photoCreatorClick?: (str: string) => void },
  ) {
    super(muya, 'ag-image-selector', options)
  ...
  listen() {
    ...
    eventCenter.subscribe(
      'muya-image-selector',
      ({ reference, cb, imageInfo }: { reference: HTMLElement; cb: () => void; imageInfo: any }) => {
        if (reference) {
          // Unselected image.
          ...
          const input = this.imageSelectorContainer.querySelector('input.src') as HTMLInputElement
          if (input) {
            input.focus()
            input.select()
          }
  ...
  loadPhotos = (first: boolean = true) => {
    ...
    this.unsplash.photos
      .list({
        page: this.unsplashQuery.pageNumber,
        perPage: this.unsplashQuery.pageSize,
      })
      .then(json => {
        this.loading = false
        if (Array.isArray(json.response?.results)) {
          this.photoList = this.photoList?.concat(json.response?.results)
  ...
  searchPhotos = (keyword: string = '', first: boolean = true) => {
    if (!this.unsplash) {
      return
    }
    if (first && keyword) {
      this.unsplashQuery.keywords = keyword
    }
    ...
    this.unsplash.search
      .getPhotos({
        query: this.unsplashQuery.keywords,
        page: this.unsplashQuery.pageNumber,
        perPage: this.unsplashQuery.pageSize,
      })
      .then(json => {
  ...
  tabClick(event: Event, tab: { value: string }) {
    const { value } = tab
    this.tab = value
    return this.render()
  }
  ...
  inputHandler(event: Event, type: 'title' | 'src' | 'alt') {
  ...
  replaceImageAsync = async ({ alt, src, title }: IState) => {
    if (!this.muya.options.imageAction || URL_REG.test(src)) {
      ...
      if (src) {
        ...
        const imageWrapper = this.muya.container.querySelector(`span[data-id=${id}]`) as HTMLElement
      ...
      if (this.loading) {
            ...
            'div.image-wrapper',
            {
              ...
              on: {
                click: () => {
                  ...
                  this.unsplash.photos.get(photo).then(json => {
                    this.unsplash.photos.trackDownload({
                      downloadLocation: json.response?.links.download_location!,
                    })
                  })
        ...
        const moreCom = h(
          'div.more',
          {
            on: {
              click: () => {
                this.unsplashQuery.pageNumber += 1
                if (this.unsplashQuery.keywords) {
