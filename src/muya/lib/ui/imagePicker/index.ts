...
class ImagePathPicker extends BaseScrollFloat<any> {
  static pluginName = 'imagePathPicker'

  renderArray: Array<IRenderItem>
  ...
  listen() {
    ...
    eventCenter.subscribe(
      'muya-image-picker',
      ({ reference, list, cb }: { reference: HTMLElement; list: Array<any>; cb: () => void }) => {
        if (list.length) {
  ...
  getItemElement(item: IRenderItem) {
    const { text } = item
