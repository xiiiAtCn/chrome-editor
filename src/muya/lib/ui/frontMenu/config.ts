...
import { IQuickInsert, IQuickObjKeys, quickInsertObj } from '../quickInsert/config'

const wholeSubMenu = (Object.keys(quickInsertObj) as Array<IQuickObjKeys>).reduce((acc, key) => {
  const items = quickInsertObj[key]
  return [...acc, ...items]
}, [] as Array<IQuickInsert>)
