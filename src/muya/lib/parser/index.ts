...
import { ITokenizerConfig, Token } from "types/muya";
import { isLengthEven, union } from "../utils";
import { IOption } from "./marked/options";
import { findClosingBracket } from "./marked/utils";
import { beginRules, inlineExtensionRules, inlineRules } from "./rules";
import {
  getAttributes,
  lowerPriority,
  parseSrcAndTitle,
...
const correctUrl = (token: Array<string>) => {
...
const tokenizerFac = (
  src: string,
  beginRules: Record<string, RegExp> | undefined | null,
...
export const tokenizer = (
  ...
  const postTokenizer = (tokens: Array<Token>) => {
...
export const generator = (tokens: Array<Token>) => {
  let result = "";
  for (const token of tokens) {
    result += token.raw;
  }
