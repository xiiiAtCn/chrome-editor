...
export default function referenceLink(
  ...
  const MARKER = '['
  const key = (label + (backlash as IRange).second).toLowerCase()
  ...
  const content = [
    ...
    ...this.backlashInToken(h, (backlash as IRange).first, className, backlashStart, token),
  ]
  ...
  const endMarker = this.highlight(
    h,
    block,
    start + MARKER.length + anchor!.length + (backlash as IRange).first.length,
    end,
    token,
  )
  ...
  if (isFullLink) {
    const labelContent = this.highlight(
      h,
      block,
      start + 3 * MARKER.length + anchor!.length + (backlash as IRange).first.length,
      end - MARKER.length - (backlash as IRange).second.length,
      token,
    )
    ...
    const secondBacklashStart = end - MARKER.length - (backlash as IRange).second.length
    ...
    return [
      ...
      ...this.backlashInToken(h, (backlash as IRange).second, className, secondBacklashStart, token),
