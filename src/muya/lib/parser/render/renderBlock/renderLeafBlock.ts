...
import { Block, IHighlight, IMatch, Token, WholeRender } from '@/typings/muya'
import katex from 'katex'
import { tokenizer } from '../..'
import { CLASS_OR_ID, DEVICE_MEMORY, HAS_TEXT_BLOCK_REG, PREVIEW_DOMPURIFY_CONFIG } from '../../../config'
import prism, { loadedCache, transfromAliasToOrigin } from '../../../prism'
import { escapeHtml, getImageInfo, getLongUniqueId, sanitize, snakeToCamel } from '../../../utils'
...
const getHighlightHtml = (text: string, highlights: Array<IHighlight>, escape = false, handleLineEnding = false) => {
  let code = ''
  let pos = 0
...
const hasReferenceToken = (tokens: Array<Token>) => {
  let result = false
  const travel = (tokens: Array<Token>) => {
