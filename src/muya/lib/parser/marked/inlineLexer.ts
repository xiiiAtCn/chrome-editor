...
export default class InlineLexer {
  ...
  rules: Record<string, { exec(str: string): RegExpExecArray | null }>
  renderer: Renderer
  ...
  constructor(links: Record<string, any>, footnotes: Map<string, IFootnote>, options: IOption) {
    ...
    for (const key of Object.keys(this.rules)) {
      if (/^(?:autolink|link|code|tag)$/.test(key) && this.rules[key] instanceof RegExp) {
        this.highPriorityEmpRules[key] = this.rules[key] as RegExp
      }
    }
    ...
    while (src) {
      ...
      if (footnote) {
        ...
        if (cap) {
          ...
          let footnoteInfo = this.footnotes.get(identifier)
          if (footnoteInfo === undefined) {
            footnoteInfo = {
              footnoteId: '',
              footnoteIdentifierId: getUniqueId(),
              order: 0,
            }
          }
  ...
  escapes(text: string) {
    return text ? text.replace(this.rules._escapes as RegExp, '$1') : text
  }

  /**
   * Compile Link
   */
  outputLink(cap: Array<string>, link: ILink) {
    const href = link.href
    const title = link.title ? encodeURIComponent(link.title) : ''
    const text = cap[1].replace(/\\([\[\]])/g, '$1') // eslint-disable-line no-useless-escape

    return cap[0].charAt(0) !== '!'
      ? this.renderer.link(href, title, this.output(text) || '')
      : this.renderer.image(href, title, encodeURIComponent(text))
  }
