...
import { Token } from '@/typings/muya'
import { gfm, normal, pedantic } from './blockRules'
import options from './options'
import { getUniqueId, rtrim, splitCells } from './utils'
...
type Null = typeof model

type IOption = typeof options

class TokenList<T> extends Array<T> {
  links: Null = null
  footnotes: Null = null

  constructor(links: any, footnote: any) {
    super()
    this.links = links
...
class Lexer {
  tokens: TokenList<Token> = new TokenList<Token>(null, null)
  footnoteOrder = 0
  rules: Record<string, RegExp> = {}
  options: Partial<IOption> = {}
  checkFrontmatter = false

  constructor(opts: Partial<IOption>) {
    this.tokens = new TokenList<Token>(Object.create(null), Object.create(null))
  ...
  lex(src: string) {
    ...
    const { tokens } = this
    const hasNoFootnoteTokens: TokenList<Token> = new TokenList<Token>(null, null)
    const footnoteTokens: TokenList<Token> = new TokenList<Token>(null, null)
    ...
    const result: TokenList<Token> = new TokenList<Token>(null, null)
    result.push(...hasNoFootnoteTokens, ...footnoteTokens)
      ...
      if (cap) {
        ...
        if (item.header!.length === item.align!.length) {
          ...
          for (i = 0; i < item.align!.length; i++) {
            if (item.align instanceof Array) {
              // @ts-ignore
        ...
        for (; i < l; i++) {
          ...
          if (!newIsOrdered && this.options.gfm) {
            ...
            if (checked) {
              ...
              tmp = tmp.replace(this.rules.checkbox, '')
              space -= 4
            } else {
              checked = undefined
            }
          ...
          if (i === 0) {
            ...
            this.tokens.push({
              ...
              ordered: isOrdered,
              listType:
                bull.length > 1 ? 'order' : /^( {0,3})([-*+]) \[[xX ]\]/.test(itemWithBullet) ? 'task' : 'bullet',
              start: isOrdered ? +bull.slice(0, -1) : '',
            })
          }
          ...
          if (~tmp.indexOf('\n ')) {
            space -= tmp.length
            tmp = !this.options.pedantic
              ? tmp.replace(new RegExp('^ {1,' + space + '}', 'gm'), '')
              : tmp.replace(/^ {1,4}/gm, '')
          }
