...
class Selection {
  ...
  findMatchingSelectionParent(testElementFunction: { (node: Node): boolean }, contentWindow: Window) {
    ...
    return traverseUp(current as HTMLElement, testElementFunction)
  }
  ...
  importSelectionMoveCursorPastAnchor(
    ...
    const nodeInsideAnchorTagFunction = function (node: Node) {
    ...
    if (
      ...
      traverseUp(range.startContainer as HTMLElement, nodeInsideAnchorTagFunction)
    ) {
  ...
  importSelectionMoveCursorPastBlocks(root: Node, index = 1, range: Range) {
    ...
    if (startContainer.nodeType === 3 && isBlockContainer(startContainer.previousSibling!)) {
      ...
      startBlock = getClosestBlockContainer(startContainer as HTMLElement)
    }
    ...
    range.setStart(getFirstSelectableLeafNode(targetNode as HTMLElement), 0)
  ...
  chopHtmlByCursor(root: Node) {
    ...
    switch (type) {
      ...
      case 'LEFT':
        return {
          pre: markedText.slice(0, left - Number(info)),
          post: markedText.slice(left - Number(info)),
        }
      case 'RIGHT':
        return {
          pre: markedText.slice(0, left + Number(info)),
  ...
  setCursorRange(cursorRange: { anchor: { key: string; offset: number }; focus: { key: string; offset: number } }) {
    ...
    const getNodeAndOffset = (node: Node, offset: number): { node: Node; offset: number } => {
      if (node.nodeType === 3) {
        return {
          node,
          offset,
    ...
    if (
      ...
      (anchorNode.nodeType === 1 && !(anchorNode as HTMLElement).classList.contains('ag-image-container'))
    ) {
  ...
  isValidCursorNode(node: HTMLElement) {
    if (!node) return false
    let tmp = node
    if (node.nodeType === 3) {
      tmp = node.parentNode as HTMLElement
    }

    return tmp!.closest('span.ag-paragraph')
  }

  getCursorRange() {
    let { anchorNode, anchorOffset, focusNode, focusOffset } = this.doc.getSelection()!
    const isAnchorValid = this.isValidCursorNode(anchorNode as HTMLElement)
    const isFocusValid = this.isValidCursorNode(focusNode as HTMLElement)
    let needFix = false
    ...
    const anchorParagraph = findNearestParagraph(anchorNode as HTMLElement)
    const focusParagraph = findNearestParagraph(focusNode as HTMLElement)

    let aOffset = getOffsetOfParagraph(anchorNode as HTMLElement, anchorParagraph!) + anchorOffset
    let fOffset = getOffsetOfParagraph(focusNode as HTMLElement, focusParagraph!) + focusOffset
    ...
    if (
      ...
      (anchorNode as HTMLElement).previousElementSibling &&
      (anchorNode as HTMLElement).previousElementSibling!.nodeName === 'IMG'
    ) {
      const imageWrapper = (anchorNode as HTMLElement).parentNode!.parentNode!
      const preElement = (imageWrapper as HTMLElement).previousElementSibling as HTMLElement
      aOffset = 0
      if (preElement) {
      ...
      aOffset += getTextContent(imageWrapper as HTMLElement, [CLASS_OR_ID.AG_MATH_RENDER, CLASS_OR_ID.AG_RUBY_RENDER])!
        .length
      fOffset = aOffset
    }
    ...
    if (
      ...
      (anchorNode as HTMLElement).classList.contains('ag-image-container')
    ) {
      ...
      if (anchorOffset === 1) {
        aOffset += getTextContent(imageWrapper as HTMLElement, [
