...
import { getParagraphReference, isLengthEven } from "../utils";

const TABLE_BLOCK_REG = /^\|.*?(\\*)\|.*?(\\*)\|/;

const tableBlockCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.tableToolBarClick = function (type) {
    ...
    switch (type) {
      ...
      case "table": {
        ...
        const reference = getParagraphReference(
          tableEle as HTMLElement,
          tableEle!.id
        );
      ...
      if (action === "insert") {
        ...
        if (location === "previous") {
          ...
          if (
            ...
            cursorBlock = (this.getNextSibling(currentRow) ||
              this.getPreSibling(currentRow))!.children![columnIndex]
              .children![0];
            this.removeBlock(currentRow);
          }
        } else {
