/** @format */
import { Block, ContentState } from '@/typings/muya'
...
const enterCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.createRow = function (row: Block, isHeader: boolean = false) {
  ...
  ContentState.prototype.enterInEmptyParagraph = function (block) {
    ...
    if (parent && /ul|ol|blockquote/.test(parent.type)) {
      ...
      if (parent.listItemType === 'task') {
        ...
        newBlock = this.createTaskItemBlock(undefined, checked)!
      } else {
  ...
  ContentState.prototype.enterHandler = function (event) {
    ...
    if (event.shiftKey && block.functionType === 'cellContent') {
      ...
      return this.partialRender(true)
    }
