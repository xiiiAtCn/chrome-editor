...
import { Block, ContentState, ISearchOption } from "types/muya";
import { defaultSearchOption } from "../config";
import Cursor from "../selection/cursor";

const matchString = (text: string, value: string, options: ISearchOption) => {
...
const searchCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.replace = function (
    replaceValue,
    opt: ISearchOption = { isSingle: true }
  ) {
    const { isSingle } = opt;
    delete opt.isSingle;
