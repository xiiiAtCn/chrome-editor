...
const coreApi = (ContentState: ContentState) => {
  /**
  ...
  ContentState.prototype.replaceWordInline = function (
    ...
    if (wordStart.key !== wordEnd.key) {
      throw new Error(
        'Expect a single line word cursor: "start.key" is not equal to "end.key".'
      );
    } else if (lineStart.key !== lineEnd.key) {
      throw new Error(
        'Expect a single line line cursor: "start.key" is not equal to "end.key".'
      );
    ...
    block!.text =
      block!.text!.substr(0, left) + replacement + block!.text!.substr(right);
