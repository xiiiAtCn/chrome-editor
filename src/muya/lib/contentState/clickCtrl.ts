...
import { CLASS_OR_ID, HAS_TEXT_BLOCK_REG } from "../config";
...
const clickCtrl = (ContentState: ContentState) => {
  ContentState.prototype.clickHandler = function (event) {
    const { eventCenter } = this.muya;
    const { target: temp } = event;
    let target = temp as HTMLElement;
    ...
    const inlineNode = node
      ? (node as HTMLElement).closest(".ag-inline-rule")
      : null;
    ...
    const needMarkedUpdate =
      ...
      this.checkNeedRender(new Cursor({ start, end }));
  ...
  ContentState.prototype.updateParentsCheckBoxState = function (checkbox) {
    let parent = getParentCheckBox(checkbox) as HTMLInputElement;
    while (parent !== null) {
      const checked = cumputeCheckboxStatus(parent);
      if (parent.checked !== checked) {
        this.setCheckBoxState(parent, checked);
        parent = getParentCheckBox(parent) as HTMLInputElement;
      } else {
        break;
      }
    }
