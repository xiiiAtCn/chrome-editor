...
import { DATA_URL_REG, URL_REG } from '../config'
import Cursor from '../selection/cursor'
import { correctImageSrc } from '../utils/getImageInfo'

const imageCtrl = (contentState: ContentState) => {
  /**
  ...
  contentState.prototype.insertImage = function ({ alt = '', src = '', title = '' }) {
    ...
    const { formats } = this.selectionFormats(new Cursor({ start, end }))
  ...
  contentState.prototype.updateImage = function ({ imageId, key, token }: Block, attrName: string, attrValue: string) {
    ...
    const image = document.querySelector(`#${imageId} img`) as HTMLElement
    if (image) {
      image.click()
  ...
  contentState.prototype.deleteImage = function ({ key, token }: Block) {
    const block = this.getBlock(key)!
    const oldText = block.text!
    const { start, end } = token!.range!
