...
// TODO: refactor later.
let renderCodeBlockTimer = -1;

const inputCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.inputHandler = function (event, notEqual = false) {
    ...
    const paragraph = document.querySelector(`#${key}`) as HTMLElement;
    ...
    if (block && (block.text !== text || notEqual)) {
      if (
        ...
        if (/^delete/.test(event.inputType)) {
          ...
          const deletedChar = block.text![offset] as "{";
          if (
    ...
    if (
      ...
      if (needRender) {
        ...
        renderCodeBlockTimer = window.setTimeout(() => {
          this.partialRender();
        }, 300);
      }
      return;
