...
import { Block, ContentState, HTMLTagToken, ImageToken, LinkToken, Token } from 'types/muya'
...
const getOffset = (offset: number, token: Token) => {
  let { range, type } = token
...
const clearFormat = (token: Token, { start, end }: Cursor) => {
  if (start) {
  ...
  switch (token.type) {
    ...
    case 'html_tag': {
      // underline, sub, sup
      const temp = <HTMLTagToken>token
      let { parent } = temp
      const index = parent!.indexOf(token)
      parent!.splice(index, 1, ...temp.children)
      break
    }
    case 'image': {
      token.type = 'text'
...
const formatCtrl = (ContentState: ContentState) => {
  ContentState.prototype.selectionFormats = function (cursor: Cursor = selection.getCursorRange()) {
      ...
      ;(function iterator(tks) {
        for (const token of tks) {
          ...
          if (token.children && token.children.length) {
            iterator(token.children as Array<Token>)
          }
        }
      })(tokens)
    }
  ...
  ContentState.prototype.clearBlockFormat = function (block: Block, { start, end } = selection.getCursorRange(), type) {
    if (!start || !end) {
      return
    }
    ...
    if (start.key === end.key && start.key === key) {
      ;({ tokens, neighbors } = this.selectionFormats(new Cursor({ start, end })))
    } else if (start.key !== end.key && start.key === key) {
      ;({ tokens, neighbors } = this.selectionFormats(
        new Cursor({ start, end: { key: start.key, offset: block.text!.length } }),
      ))
    ...
    for (const neighbor of neighbors) {
      clearFormat(neighbor, new Cursor({ start, end }))
    }
    ...
    if (start.key === end.key) {
      ...
      if (type === 'clear') {
        ...
        for (const token of currentFormats) {
          clearFormat(token, new Cursor({ start, end }))
        }
        start.offset += start.delata
        end.offset += end.delata
        startBlock.text = generator(tokens)
        ...
        if (type === 'image') {
          ...
          requestAnimationFrame(() => {
            const startNode = selection.getSelectionStart() as HTMLElement
            if (startNode) {
      ...
      while (nextBlock && nextBlock !== endBlock) {
        this.clearBlockFormat(nextBlock, new Cursor({ start, end }), formatType)
        nextBlock = this.findNextBlockInLocation(nextBlock)!
      }
      this.clearBlockFormat(endBlock!, new Cursor({ start, end }), formatType)
