...
const calculateAspects = (tableId: string, barType: "button" | string) => {
...
export const getAllTableCells = (tableId: string) => {
  ...
  const cells: Array<Array<HTMLElement>> = [];
  for (const row of Array.from(rows)) {
    cells.push(Array.from(row.children as unknown as Array<HTMLElement>));
  }
...
export const getIndex = (barType: string, cell: HTMLElement) => {
  ...
  if (barType === "bottom") {
    ...
    if (rowContainer.tagName === "THEAD") {
      ...
      return Array.from(rowContainer.children).indexOf(row as HTMLElement) + 1;
    }
  }
};
...
const tableDragBarCtrl = (contentState: ContentState) => {
  contentState.prototype.handleMouseDown = function (event: MouseEvent) {
    event.preventDefault();
    const { eventCenter } = this.muya;
    const { clientX, clientY, target: tmp } = event;
    let target = tmp as HTMLElement;
    ...
    mouseMoveId && this.dragEventIds.push(mouseMoveId);
    mouseUpId && this.dragEventIds.push(mouseUpId);
  };
  ...
  contentState.prototype.hideUnnecessaryBar = function () {
    ...
    const needHideBar = document.querySelector(
    ...
    if (index !== curIndex) {
      ...
      if (barType === "bottom") {
        for (const row of rows) {
          const isCursorCell =
            row.children![index]!.children![0].key === start.key;
          ...
          for (i = index; i < curIndex; i++) {
            rows[i].children!.forEach((cell, ii) => {
              cell.children![0].text =
                rows[i + 1].children![ii].children![0].text;
            });
          }
