...
const pasteCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.checkCopyType = function (html, text) {
    ...
    if (!html && text) {
      ...
      if (match && match[1]) {
        ...
        type = PARAGRAPH_TYPES.find((type) => type === tag)
          ? "copyAsHtml"
          : type;
      }
    }
  ...
  ContentState.prototype.pasteImage = async function (event) {
    ...
    if (file) {
      ...
      const nSrc = await this.muya.options.imageAction(file, id, file.name);
  ...
  ContentState.prototype.pasteHandler = async function (
    ...
    const getLastBlock = (blocks: Array<Block>): Block => {
          ...
          if (targetListType !== originListType) {
            if (!targetListType) {
              firstFragment.children!.forEach(
                (item) => (item.isLooseListItem = true)
              );
            } else {
