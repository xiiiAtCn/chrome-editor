...
import { BackSpaceBlock, Block, ContentState } from "types/muya";
...
const backspaceCtrl = (contentState: ContentState) => {
  ...
  contentState.prototype.backspaceHandler = function (event) {
    ...
    if (
      ...
      if (
        ...
        const figureBlock = this.getBlock(
          this.closest(startBlock, "figure")?.key
        )!;
    ...
    const inlineDegrade = this.checkBackspaceCase() as BackSpaceBlock;
    // Handle backspace when the previous is an inline image.
    if (
      parentNode &&
      (parentNode as HTMLElement).classList.contains("ag-inline-image")
    ) {
