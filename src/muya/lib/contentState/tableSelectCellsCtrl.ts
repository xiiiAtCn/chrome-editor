...
const tableSelectCellsCtrl = (contentState: ContentState) => {
  ...
  contentState.prototype.handleCellMouseMove = function (event: Event) {
    const { target: tmp } = event;
    let target = tmp as HTMLElement;
    const cell = target.closest("th") || target.closest("td")!;
    const table = target.closest("table");
    const isOverSameTableCell =
      cell && table && table.id === this.cellSelectInfo!.tableId;
  ...
  contentState.prototype.handleCellMouseUp = function (event) {
    ...
    for (const id of this.cellSelectEventIds) {
      eventCenter.detachDOMEvent(id as string);
    }
  ...
  contentState.prototype.selectTable = function (table) {
    ...
    this.selectedTableCells = {
      ...
      cells: this.cellSelectInfo.selectedCells!.map((c: any) => {
        delete c.ele;
        return c;
      }),
    };
  ...
  contentState.prototype.isWholeTableSelected = function () {
    const { selectedTableCells } = this;
    const table = selectedTableCells
      ? this.getBlock(selectedTableCells.tableId)
      : null;
    if (table) {
