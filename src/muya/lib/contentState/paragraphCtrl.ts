...
import { DEFAULT_TURNDOWN_CONFIG, PARAGRAPH_TYPES } from "../config";
...
const paragraphCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.updateParagraph = function (
    ...
    if (
      !this.isAllowedTransformation(block!, paraType, start.key !== end.key)
    ) {
      return;
    }
    ...
    switch (paraType) {
      ...
      case "paragraph": {
        ...
        // \u00A0 is &nbsp;
        const [, hash, partText] = /(^ {0,3}#*[ \u00A0]*)([\s\S]*)/.exec(
          text!
        )!;
