...
const containerCtrl = (ContentState: ContentState) => {
  ContentState.prototype.createContainerBlock = function (
    ...
    if (functionType === "multiplemath") {
      if (style === undefined) {
        figureBlock.mathStyle = this.isGitlabCompatibilityEnabled
          ? "gitlab"
          : "";
      }
  ...
  ContentState.prototype.createPreAndPreview = function (
    functionType,
    value = ""
  ) {
    const lang =
  ...
  ContentState.prototype.updateMathBlock = function (block): boolean {
