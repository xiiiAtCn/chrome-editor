...
import { Block, ContentState } from "types/muya";
...
const arrowCtrl = (contentState: ContentState) => {
  contentState.prototype.findNextRowCell = function (
    cell: Block
  ): Block | null {
    if (cell.functionType !== "cellContent") {
      throw new Error(
        `block with type ${cell && cell.type} is not a table cell`
      );
    }
    ...
    if (rowContainer!.type === "thead") {
      ...
      if (tbody && tbody.children && tbody.children.length) {
        ...
        return child.children![column]!.children![0];
      }
  ...
  contentState.prototype.findPrevRowCell = function (cell: Block) {
    if (cell.functionType !== "cellContent") {
      throw new Error(
        `block with type ${cell && cell.type} is not a table cell`
      );
  ...
  contentState.prototype.docArrowHandler = function (event: KeyboardEvent) {
    ...
    if (
      ...
      if (right === 0 && start.key === end.key && start.offset === end.offset) {
        ...
        return selection.select(
          (node.parentNode! as HTMLElement).nextElementSibling!,
          0
        );
      }
    }
