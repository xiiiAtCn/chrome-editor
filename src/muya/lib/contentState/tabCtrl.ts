/** @format */
// @ts-nocheck
import { ContentState, Token } from "types/muya";
import { HAS_TEXT_BLOCK_REG, HTML_TAGS, VOID_HTML_TAGS } from "../config";
...
const tabCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.checkCursorAtEndFormat = function (text, offset) {
    ...
    let result = null;
    const walkTokens = (tkns: Array<Token>) => {
      for (const token of tkns) {
        ...
        if (children && children.length) {
          walkTokens(children as Array<Token>);
        }
      }
    };
    walkTokens(tokens);
  ...
  ContentState.prototype.tabHandler = function (event) {
    ...
    if (event.shiftKey && !(startBlock.functionType === "cellContent")) {
      ...
      if (unindentType) {
        this.unindentListItem(startBlock, unindentType as string);
      }
      return;
    }
