...
const updateCtrl = (ContentState: ContentState) => {
  ...
  ContentState.prototype.checkNeedRender = function (cursor) {
    if (!cursor) {
      cursor = this.cursor;
    }
  ...
  ContentState.prototype.updateList = function (
    ...
    if (TASK_LIST_REG.test(listItemText!)) {
      const [, , tasklist, , , ,] =
        listItemText!.match(INLINE_UPDATE_REG) || []; // eslint-disable-line comma-spacing
      return this.updateTaskListItem(block, "tasklist", tasklist);
    } else {
      return block;
    }
