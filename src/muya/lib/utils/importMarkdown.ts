...
import { Block, ContentState, Token } from "@/typings/muya";
...
const turnSoftBreakToSpan = (html: string) => {
  ...
  const travel = (childNodes: NodeListOf<ChildNode>) => {
    for (const node of childNodes) {
      if (
        node.nodeType === 3 &&
        (node.parentNode! as HTMLElement).tagName !== "CODE"
      ) {
        let startLen = 0;
        let endLen = 0;
        const text = node
...
const importRegister = (ContentState: ContentState) => {
  // turn markdown to blocks
  ContentState.prototype.markdownToState = function (markdown: string) {
    // mock a root block...
    ...
    while ((token = tokens.shift())) {
      switch (token.type) {
        ...
        case "list_start": {
          ...
          if (listType === "order") {
            block.start = (/^\d+$/.test(start as string) ? start : 1) as number;
          }
          this.appendChild(parentList[0], block);
          parentList.unshift(block);
          break;
  ...
  ContentState.prototype.importCursor = function (hasCursor) {
    ...
    if (hasCursor) {
      ...
      cursor.anchor = cursor.start = { key, offset };
      cursor.focus = cursor.end = { key, offset };
    }
