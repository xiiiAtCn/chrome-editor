...
import { IOption as Config } from '../parser/marked/options'
import loadRenderer from '../renderers'
import { validEmoji } from '../ui/emojis'

export const getSanitizeHtml = (markdown: string, options: Partial<Config>) => {
  const html = marked(markdown, options)
  return sanitize(html, EXPORT_DOMPURIFY_CONFIG, false)
}

// const githubMarkdownCss = require('github-markdown-css/github-markdown.css').toString()
const katexCss = require('katex/dist/katex.css').toString()
const exportStyle = require('../assets/styles/exportStyle.css').toString()
const highlightCss = require('prismjs/themes/prism.css')
const footerHeaderCss = require('../assets/styles/headerFooterStyle.css').toString()
...
class ExportHtml {
  ...
  constructor(markdown: string, muya: Muya) {
  ...
  mathRenderer = (math: string, displayMode: boolean) => {
   ...
   */
  async generate(options: IOption) {
...
<body>
   ...
   */
  _prepareHtml(html: string, options: IOption) {
...
const createRealFooter = (options: IOption) => {
...
const getHeaderFooterStyledClass = (value?: boolean) => {
  if (value === undefined) {
