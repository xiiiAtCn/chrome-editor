  ...
  clickBinding() {
    ...
    const handler = (event: Event) => {
      ...
      if (toolItem) {
        ...
        if ((grandPa! as HTMLElement).classList.contains('ag-tool-table')) {
      ...
      const imageWrapper = target.closest<HTMLElement>(`.${CLASS_OR_ID.AG_INLINE_IMAGE}`)
      ...
      if (markedImageText && markedImageText.classList.contains(CLASS_OR_ID.AG_IMAGE_MARKED_TEXT)) {
        eventCenter.dispatch('format-click', {
          event,
          formatType: 'image',
          data: (event.target! as HTMLElement).getAttribute('src'),
        })
      ...
      const editIcon = target.closest<HTMLElement>('.ag-container-icon')
      if (editIcon) {
        event.preventDefault()
        event.stopPropagation()
        if ((editIcon!.parentNode! as HTMLElement).classList.contains('ag-container-block')) {
          contentState.handleContainerBlockClick(editIcon.parentNode as HTMLElement)
        }
      }

      // handler to-do checkbox click
      if (target.tagName === 'INPUT' && target.classList.contains(CLASS_OR_ID.AG_TASK_LIST_ITEM_CHECKBOX)) {
        contentState.listItemCheckBoxClick(target as HTMLInputElement)
      }
      contentState.clickHandler(event as MouseEvent)
    }
...
function getToolItem(target: HTMLElement) {
