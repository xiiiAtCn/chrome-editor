...
import { getImageInfo, getParagraphReference } from '../utils'
...
class Keyboard {
  ...
  listen() {
    ...
    this.muya.eventCenter.subscribe('muya-float', (tool: { name: string; hide: () => void }, status: boolean) => {
  ...
  recordIsComposed() {
    ...
    const handler = (event: CompositionEvent) => {
      if (event.type === 'compositionstart') {
        ...
        contentState.inputHandler((event as any) as InputEvent)
  ...
  keydownBinding() {
    ...
    eventCenter.attachDOMEvent(document.scrollingElement as HTMLElement, 'keydown', docHandler)
  }
  ...
  inputBinding() {
    ...
    const inputHandler = (event: Event) => {
      if (!this.isComposed) {
        contentState.inputHandler(event as InputEvent)
        this.muya.dispatchChange()
      }
      ...
      if (lang) {
        eventCenter.dispatch('muya-code-picker', {
          reference: getParagraphReference(paragraph! as HTMLElement, paragraph!.id),
          lang,
          cb: (item: { name: string }) => {
            contentState.selectLanguage(paragraph as HTMLElement, item.name)
          },
        })
      } else {
  ...
  keyupBinding() {
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      // check if edit emoji
      const node = selection.getSelectionStart()! as HTMLElement
      ...
      if (!this.isComposed) {
        ...
        if (
          ...
          const needRender =
            ...
            contentState.checkNeedRender(new Cursor({ start, end }))
