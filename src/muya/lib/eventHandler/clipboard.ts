...
class Clipboard {
  muya: Muya
  _copyType: 'normal' | 'copyAsMarkdown' | 'copyAsHtml' | 'copyCodeContent' | 'copyBlock'
  _pasteType: 'normal' | 'pasteAsPlainText'
  _copyInfo: any
  ...
  listen() {
    ...
    eventCenter.attachDOMEvent(document.scrollingElement as HTMLElement, 'paste', docPasteHandler)
   ...
   */
  copy(type: 'normal' | 'copyAsMarkdown' | 'copyAsHtml' | 'copyCodeContent' | 'copyBlock', info: any) {
    this._copyType = type
    this._copyInfo = info
    document.execCommand('copy')
  }
